...
```{r, fig.height=10, fig.width=15, paged.print=TRUE}
basemap <- leaflet() %>%
  addProviderTiles(
    "CartoDB.Positron",
    group = "CartoDB"
...
```{r}
zoom_layer = zoom_basemap %>% addLayersControl(baseGroups = c('Topology', "OpenStreetMap","CartoDB"),
                                  position = 'topleft')
```
...
```{r}
map_marker = basemap %>% addMarkers(lng = parks$x, lat = parks$y,label = parks$park_name)
```
...
base_marker = basemap %>% addMarkers(lng = parks$x, lat = parks$y, icon = icons_list, label = parks$park_name)
```
...
```{r}
base_marker = basemap %>% addMarkers(lng = parks$x, lat = parks$y, icon = icons_list, popup = parks$park_name )
```
...
```{r}
leaflet_tf_water = leaflet(data = tf_water) %>%
...
map_polygon = leaflet() %>%
  ...
  addPolygons(data = water_pond,  color = "blue", label = water_pond$NAME, fillOpacity = 1)
```
## Add Lines
